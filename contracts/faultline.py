...
def _incident_consensus(evaluate_once) -> dict:
    def leader_fn() -> dict:
        return _incident_result(evaluate_once())

    def validator_fn(leader_result: gl.vm.Result) -> bool:
        if not isinstance(leader_result, gl.vm.Return):
            return False
        try:
            own = _incident_result(evaluate_once())
            proposed = _incident_result(leader_result.calldata)
            fields = (
                "affected_release",
...
class Faultline(gl.Contract):
    ...
    def register_release(
        ...
        release_id = "fl-rel-" + str(int(self.next_release))
    ...
    def open_warranty(
        ...
        warranty_id = "fl-war-" + str(int(self.next_warranty))
    ...
    def open_incident(self, warranty_id: str, title: str, advisory_hint: str) -> str:
        ...
        incident_id = "fl-inc-" + str(int(self.next_incident))
    ...
    def commit_evidence(self, incident_id: str, commitment: str) -> str:
        ...
        evidence_id = "fl-ev-" + str(int(self.next_evidence))
        ...
        def evaluate_once() -> dict:
            ...
            if not content.strip():
                return {
                    ...
                    "advisory_id": "",
                    "affected_range": "",
                    "basis": "The source returned no usable text.",
                }
            content = content[:MAX_SOURCE_CONTENT]
            prompt = f"""
FAULTLINE_SOURCE_EXAMINER_V1
...
"""
        ...
        result = _source_result(gl.vm.run_nondet_unsafe(evaluate_once, validator_fn))
...
"""
        ...
        result = _incident_result(gl.vm.run_nondet_unsafe(evaluate_once, validator_fn))
